
module ALU (
    input logic clk,
    input logic reset,      
    input logic mac_request,   
    input logic [31:0] a,          // First operand
    input logic [31:0] b,          // Second operand
    input logic [31:0] c,          // Third operand (for MAC)
    input logic [3:0] alu_control, // ALU control signal

    output logic [31:0] result,    // ALU result
    output logic zero               // Zero flag
);

    logic mac_active_q = 1'b0;
    logic mac_done = 1'b0;

    always_ff @(posedge clk or posedge reset) begin
        if(reset) begin
            mac_active_q <= 1'b0;
            mac_done <= 1'b0;
        end else begin
            mac_active_q <= mac_request;
            mac_done <= mac_active_q;
        end
    end

    logic [47:0] accumulator = 48'h0;
    logic [47:0] result_dsp;
    
    // DSP operations
    // This dsp module is for MAC (Multiply and accumulate) instructions
    // A * B + C = P
    MAC_dsp mac_inst (
        .CLK(clk),
        .A(a[24:0]),
        .B(b[17:0]),
        .C(accumulator),
        .P(result_dsp)
    );

    // Find a way to properly record the C value on operations
    // Is it truly valuable to use this MAC? or to keep it on MUL and ADD? 
    // Maybe add new registers, and new instructions
    always_ff @(negedge clk or posedge reset) begin
        if (reset)
            accumulator <= 48'h0;
        else if (mac_done)
            accumulator <= result_dsp;  // result is valid now
    end

    always_comb begin
        unique case (alu_control)
            4'b0000: result = a & b;   
            4'b0001 : result = a | b;
            4'b0010: result = a + b;   // Addition
            4'b0011: result = a << b[4:0]; // Shift left logical
            4'b0100: result = a ^ b;   // XOR
            4'b0101: result = a >> b[4:0]; // Shift right logical
            4'b0110: result = a - b;   // Subtraction
            4'b0111: result = (a < b) ? 32'b1 : 32'b0; // Set less than
            4'b1001: result = (a < b) ? 32'b1 : 32'b0; // Set less than unsigned
            4'b1010: result = a >>> b[4:0]; // Shift right arithmetic
            4'b1011: result = result_dsp[31:0]; // MAC 
            4'b1100: result = 32'b0; 
            4'b1101: result = 32'b0;
            
            default: result = 32'b0;   // Default case
        endcase
        
        zero = (result == 32'b0); // Set zero flag if result is zero
    end
endmodule